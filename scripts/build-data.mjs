...
function mangaFigure(file, alt) {
  const src = `/manga/ue/${encodeURIComponent(file)}`;
  return `<figure class="manga-panel"><img src="${src}" alt="${escapeHtml(alt)}" loading="lazy" decoding="async" /></figure>`;
}

/** 各中見出しの直後に漫画を差し込む */
function injectMangaAfterH2(html, rows) {
  let out = html;
...
function injectGendaigoManga8(html) {
  const fig = mangaFigure('8_鵜葺草葺合へずの命.png', '漫画・鵜葺草葺不合の命');
  const patterns = [
...
function buildKochu() {
  ...
  if (kJo && fs.existsSync(MANGA_UE_SRC)) {
    kJo.html = injectMangaAfterH2(kJo.html, KOCHU_UE_MANGA);
  }
...
function buildGendaigo() {
  ...
  if (gJo && fs.existsSync(MANGA_UE_SRC)) {
    gJo.html = injectMangaAfterH2(gJo.html, GENDAIGO_UE_MANGA);
